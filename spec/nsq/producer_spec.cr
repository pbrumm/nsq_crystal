  ...
  describe "connecting directly to a single nsqd" do
    ...
    describe "#write" do
      it "can queue a message" do
        ...
        expect(producer.topic_msgcount("topic")).to eq(0)
        producer.write("some-message")
        loop do
          if producer.topic_msgcount("topic") == 1
            break
          else
            sleep 0.02
          end
      ...
      #       messages_received = Array(String).new
      ...
      it "can send multiple message with unicode characters" do
        ...
        3.times do
          ...
          # p [:msg, msg]
          if msg
            expect(msg.as(Nsq::Message).body).to eq("☺")
            msg.as(Nsq::Message).finish
          end
        end
        # end
        # consumer.terminate
